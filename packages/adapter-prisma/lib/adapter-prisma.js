...
class PrismaAdapter extends BaseKeystoneAdapter {
  ...
  async _connect({ rels }) {
    ...
    // return new prismaClient.PrismaClient({ log: ['query'] });
  ...
  async dropDatabase() {
    ...
    for (const { tablename } of await this.prisma.queryRaw(
      ...
      await this.prisma.queryRaw(`TRUNCATE TABLE \"${this.schemaName}\".\"${tablename}\" CASCADe;`);
    }
    if (migrationNeeded) {
  ...
  _runPrismaCmd(cmd) {
    return execSync(`yarn prisma ${cmd} --schema ${this.prisma.engineConfig.datamodelPath}`, {
      env: {
        ...process.env,
